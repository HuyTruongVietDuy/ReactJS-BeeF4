...
const ViewCart = () => {
  ...
  const increaseQuantity = (product) => {
    dispatch(SuaSL({ id_sp: product.id_sp, id_mau: product.id_mau, soluong: product.soluong + 1 }));
  };
              ...
              {cart.map((product, index) => (
                <tr key={index}>
                  ...
                  <td className="td td-4">
                    <i className="material-icons" onClick={() => dispatch(XoaSP({ id_sp: product.id_sp, id_mau: product.id_mau }))}>delete</i>
                  </td>
                </tr>
              ))}
            </tbody>
